...
FSS 全栈脚手架的多语言后端目录，支持 Java、Go、Rust、Python 四种实现。
...
| `admin-go/` | 极简 Go 后端（零外部依赖）。入口: `main.go` |
| `admin-rust/` | 极简 Rust 后端。入口: `src/main.rs` |
| `agent-python/` | Python 后端。入口: `main.py`，使用 `agent-python/.venv` 虚拟环境 |
...
| Python 后端 | `agent-python/` | Python 后端，使用 `agent-python/.venv` 虚拟环境 |
...
- **Python**: 使用 `agent-python/.venv` 虚拟环境（`.venv/Scripts/python`），运行 `python main.py`
