...
#for(column: table.getSortedColumns())
    #(column.buildComment())
    public static final QueryColumn #(tableDefConfig.buildFieldName(column.property)) = new QueryColumn(TABLE_NAME, "#(column.name)");
     ...
     */
    public static final QueryColumn[] #(tableDefConfig.buildFieldName("defaultColumns")) = new QueryColumn[]{#for(column: table.columns)#if(column.isDefaultColumn())#(tableDefConfig.buildFieldName(column.property))#if(for.index + 1 != for.size), #end#end#end};
    ...
    public #(tableDefClassName) as(String alias) {
        ...
        return getCache(key, k -> new #(tableDefClassName)("#(schema)", TABLE_NAME, alias));
    }
